...
export function VenuesPageClient({
  ...
  const baseSections = sections.length > 0 ? sections : initialSections;

  // Filter the venues-grid section features by category
  const filteredSections = useMemo(() => {
    if (activeFilter === "All" || isEditMode) return baseSections;
  ...
  }, [baseSections, activeFilter, isEditMode]);

  return (
    <div>
      {isEditMode && filteredSections.length === 0 && (
        <AddSectionButton
          onAddSection={(type) => {
