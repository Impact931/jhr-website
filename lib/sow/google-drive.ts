...
/**
 * Export a Google Drive file as PDF and return the PDF buffer.
 */
export async function exportDriveFileAsPdf(fileId: string): Promise<Buffer | null> {
  const accessToken = await getGoogleAccessToken(IMPERSONATE_EMAIL, SCOPES);
  if (!accessToken) {
    console.error('Failed to get Google Drive access token for PDF export');
    return null;
  }

  const res = await fetch(
    `${DRIVE_FILES_URL}/${fileId}/export?mimeType=application/pdf`,
    {
      method: 'GET',
  ...
  if (!res.ok) {
    const err = await res.text();
    console.error('Google Drive PDF export failed:', res.status, err);
    return null;
  }
